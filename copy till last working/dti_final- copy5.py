...
#X_res, Y_res = sm.fit_resample(xp, Y)

ros = RandomOverSampler(random_state=42)
X_res, Y_res = ros.fit_resample(xp, Y)
counter = Counter(Y_res)
print('OverSampling dataset shape %s' % Counter(Y_res))
'''
...
X_res1, Y_res1 = rus.fit_resample(yp, y1)
counter1 = Counter(Y_res1)
...
for label, _ in counter1.items():
	row_ix = where(Y_res1  == label)[0]
	pyplot.scatter(X_res1[row_ix, 0], X_res1[row_ix, 1], label=str(label))
pyplot.legend()
pyplot.show()
print(len(X_res1))
print(y1)
...
X_train,X_test,y_train,y_test=train_test_split(X_res2,y_res2, test_size=0.2, random_state=31)
rf = RandomForestClassifier(max_depth=2, random_state=0)
rf.fit(X_train, y_train)
result1 = rf.predict(X_test)
print("Accuracy : ",metrics.accuracy_score(y_test,result1))
